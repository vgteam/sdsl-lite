...
hugepage_allocator::hsbrk(size_t size)
{
    ...
    if (left < (ptrdiff_t) size) {  // enough space left?
        SDSL_THROW(std::system_error(ENOMEM, std::system_category(), "hugepage_allocator: not enough hugepage memory available"));
    }
...
hugepage_allocator::determine_available_hugepage_memory()
{
    ...
    if (mifs.is_open()) {
        ...
        SDSL_THROW(std::system_error(ENOMEM, std::system_category(), "hugepage_allocator could not automatically determine available hugepages"));
    }
